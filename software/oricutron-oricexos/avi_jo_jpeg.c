...
#define _CRT_SECURE_NO_WARNINGS // suppress warnings about mem_fopen()
#endif
...
bool jo_write_jpg(const char* filename, const void* data, int width, int height, int comp, int quality)
{
  ...
  mem_fwrite(head0, sizeof(head0), 1, fp);
  mem_fwrite(YTable, sizeof(YTable), 1, fp);
  mem_putc(1, fp);
  mem_fwrite(UVTable, sizeof(UVTable), 1, fp);
  ...
  mem_fwrite(head1, sizeof(head1), 1, fp);
  mem_fwrite(std_dc_luminance_nrcodes+1, sizeof(std_dc_luminance_nrcodes)-1, 1, fp);
  mem_fwrite(std_dc_luminance_values, sizeof(std_dc_luminance_values), 1, fp);
  mem_putc(0x10, fp); // HTYACinfo
  mem_fwrite(std_ac_luminance_nrcodes+1, sizeof(std_ac_luminance_nrcodes)-1, 1, fp);
  mem_fwrite(std_ac_luminance_values, sizeof(std_ac_luminance_values), 1, fp);
  mem_putc(1, fp); // HTUDCinfo
  mem_fwrite(std_dc_chrominance_nrcodes+1, sizeof(std_dc_chrominance_nrcodes)-1, 1, fp);
  mem_fwrite(std_dc_chrominance_values, sizeof(std_dc_chrominance_values), 1, fp);
  mem_putc(0x11, fp); // HTUACinfo
  mem_fwrite(std_ac_chrominance_nrcodes+1, sizeof(std_ac_chrominance_nrcodes)-1, 1, fp);
  mem_fwrite(std_ac_chrominance_values, sizeof(std_ac_chrominance_values), 1, fp);
  static const unsigned char head2[] = { 0xFF,0xDA,0,0xC,3,1,0,2,0x11,3,0x11,0,0x3F,0 };
  mem_fwrite(head2, sizeof(head2), 1, fp);
