...
static int avi_update_header(avi_t* AVI)
{
  int njunk, hasIndex, ms_per_frame, frate, flag; // sampsize
  ...
  for(j=0; j<AVI->anum; ++j)
  {
    ...
    OUTLONG(0);             /* Length */
    OUTLONG(0);             /* SuggestedBufferSize */
    OUTLONG(10000);         /* Quality */

    // ThOe /4
    OUTLONG(2);             /* SampleSize */
    ...
    // ThOe
    OUTLONG(AVI->track[j].a_rate*2);        /* SamplesPerSec */
    //ThOe (/4)

    OUTSHRT(2);                             /* BlockAlign */
...
static int avi_close_output_file(avi_t* AVI)
{
  ...
  int ret, njunk, hasIndex, ms_per_frame, frate, idxerror, flag; // sampsize,
...
int AVI_dup_frame(avi_t* AVI)
{
  ...
  if(AVI->compressed)
    rc = avi_add_index_entry(AVI,(unsigned char*)"00dc",0x10,AVI->last_pos,AVI->last_len);
  else
    rc = avi_add_index_entry(AVI,(unsigned char*)"00db",0x10,AVI->last_pos,AVI->last_len);
